...
mkParserTestGroup groupName parser cases =
  testGroup
    groupName
...
evaluationTests = testGroup "evaluation tests" (mkTest <$> evaluationTestCases)
  where
    ...
    evaluationTestCases =
      ...
      , ([Rule (EEQ EVarDescription (EText "Purchasing Foos at Bar Inc")) account3], Just account3)
      , ([Rule (EMatch EVarDescription (EText "^Purchasing")) account3], Just account3)
      ...
      , ([Rule (EGT EVarBookingDate (EDate $ fromGregorian 2017 12 31)) account1], Just account1)
      , ([Rule (ELT EVarBookingDate (EDate $ fromGregorian 2017 12 31)) account1], Nothing)
      ]
